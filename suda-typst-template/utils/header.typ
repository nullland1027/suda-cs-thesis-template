...
#let no-numbering-page-header(
  ...
  set page(header: context {
    ...
    let headerText = if title != "" { title } else {
      if doctype == "doctor" {
        "苏州大学博士学位论文"
    ...
    // 英文摘要页使用英文标题
    if title_en != "" and (headingTitle == [Abstract] or headingTitle == [ABSTRACT]) {
      headerText = title_en
    }
  ...
  set page(header: context {
    ...
    if elems.len() != 0 {
      ...
      headingNumber = if doctype == "bachelor" {
        "第" + numbering("一", counter(heading).get().first() + 1) + "章"
      } else {
      ...
      elems = query(selector(heading.where(level: 1)).before(here()))
      headingTitle = elems.last().body
      headingNumber = if doctype == "bachelor" { "第" + numbering("一", counter(heading).get().first()) + "章" } else {
